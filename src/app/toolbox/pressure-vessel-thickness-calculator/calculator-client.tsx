    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          <div>
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Vessel Type</label>
            <div className="grid grid-cols-2 gap-2">
              {(['cylindrical', 'spherical'] as const).map(t => (
                <button key={t} onClick={() => setVesselType(t)}
                  className={`px-3 py-2.5 rounded-xl text-base font-black border transition-all ${vesselType === t ? 'bg-red-600 border-red-600 text-white' : 'bg-white border-slate-100 text-slate-600 hover:border-slate-200'}`}>
          ...
          <div>
            ...
            <select value={matIdx} onChange={e => setMatIdx(parseInt(e.target.value))}
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-lg font-bold focus:outline-none focus:ring-4 focus:ring-red-600/5">
          ...
          {matIdx === 4 && (
            <div>
              <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Allowable Stress (MPa)</label>
          ...
          <div>
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Inside Diameter (mm)</label>
            <input type="number" value={diameter} onChange={e => setDiameter(parseFloat(e.target.value) || 0)}
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-lg font-bold" />
          </div>

          <div>
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Design Pressure (bar)</label>
            <input type="number" value={pressure} onChange={e => setPressure(parseFloat(e.target.value) || 0)}
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-lg font-bold" />
          </div>

          <div>
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Joint Efficiency (E)</label>
          ...
          <div>
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Corrosion Allowance (mm)</label>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-red-50 rounded-2xl p-6 border border-red-100">
                <div className="text-base font-black text-red-400 uppercase tracking-wider mb-2">Min. Wall Thickness</div>
                <div className="text-3xl font-black text-red-700">{result.t.toFixed(2)}<span className="text-lg text-red-400"> mm</span></div>
                <div className="text-base text-red-600 font-medium mt-1">Calculated (excl. CA)</div>
              </div>
              <div className="bg-orange-50 rounded-2xl p-6 border border-orange-100">
                <div className="text-base font-black text-orange-400 uppercase tracking-wider mb-2">Total Thickness</div>
                ...
                <div className="text-base text-orange-600 font-medium mt-1">Including {corrosion}mm CA</div>
              </div>
            </div>
            ...
            <div className="mt-4 bg-slate-50 rounded-xl p-4 border border-slate-100">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-base font-black text-slate-400 uppercase mb-1">MAWP (bar)</div>
                  <div className="text-xl font-black text-slate-700">{result.MAWP.toFixed(2)}</div>
                </div>
                <div>
                  <div className="text-base font-black text-slate-400 uppercase mb-1">Allowable Stress</div>
            ...
            <div className="mt-6 bg-amber-50 rounded-2xl p-4 border border-amber-100">
              <div className="flex items-start gap-2 text-base text-amber-700 font-medium">
