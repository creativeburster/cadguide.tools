    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Fluid Type</label>
            <div className="grid grid-cols-2 gap-2">
              {(['liquid', 'gas'] as const).map(t => (
                <button key={t} onClick={() => setFluidType(t)}
                  className={`px-3 py-2.5 rounded-xl text-xs font-black border transition-all ${fluidType === t ? 'bg-teal-600 border-teal-600 text-white' : 'bg-white border-slate-100 text-slate-600'}`}>
                  {t === 'liquid' ? 'Liquid' : 'Gas'}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Specific Gravity</label>
            <input type="number" step="0.01" value={sg} onChange={e => setSg(parseFloat(e.target.value) || 0)}
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold" />
          </div>

          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Inlet Pressure P1 (bar)</label>
            <input type="number" step="0.1" value={p1} onChange={e => setP1(parseFloat(e.target.value) || 0)}
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold" />
          </div>

          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Outlet Pressure P2 (bar)</label>
          ...
          {fluidType === 'gas' && (
            <div>
              <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Temperature (°C)</label>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            ...
            <div className="bg-teal-50 rounded-2xl p-6 border border-teal-100 mb-4">
              <div className="text-xs font-black text-teal-400 uppercase tracking-wider mb-2">Required Cv</div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                <div className="text-xs font-black text-slate-400 uppercase mb-1">Pressure Drop ΔP</div>
                <div className="text-lg font-black text-slate-700">{result.dP.toFixed(2)} bar</div>
              </div>
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                <div className="text-xs font-black text-slate-400 uppercase mb-1">Flow Rate</div>
