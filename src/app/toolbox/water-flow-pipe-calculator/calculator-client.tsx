...
export default function WaterFlowPipeCalculatorClient() {
  const [inputs, setInputs] = useState(["50","10","50","0.02"]);
  ...
  const result = useMemo(() => {
    const D=parseFloat(inputs[0])/1000,H=parseFloat(inputs[1]),L=parseFloat(inputs[2]),f=parseFloat(inputs[3]);const g=9.81;const v=Math.sqrt(2*g*H/(1+f*L/D));const A=Math.PI*D*D/4;const Q=v*A*1000;const Re=1000*v*D/0.001;return{Q,v,Re};
  }, [inputs]);
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          <button onClick={() => setInputs(["50","10","50","0.02"])}
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-orange-50 rounded-2xl p-6 border border-orange-100">
                <div className="text-base font-black text-orange-400 uppercase tracking-wider mb-2">Reynolds</div>
                <div className="text-3xl font-black text-orange-700">{formatNum(result.Re)}</div>
              </div>
            </div>
