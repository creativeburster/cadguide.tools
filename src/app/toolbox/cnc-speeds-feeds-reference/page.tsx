    ...
    <>
      ...
      <main className="min-h-screen bg-slate-50 print:bg-white print:min-h-0">
        ...
        <section className="bg-white border-b border-slate-100 py-6 print:hidden">
          <div className="max-w-[1200px] mx-auto px-6 md:px-12">
            <h2 className="text-sm font-black text-slate-400 uppercase tracking-wider mb-3">Core Formulas</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-slate-50 rounded-2xl border border-slate-100 p-4">
                <div className="text-xs font-black text-slate-400 uppercase mb-2">Spindle Speed (RPM)</div>
                <div className="font-mono text-sm text-slate-800 font-bold">RPM = (Vc × 1000) / (π × D)</div>
                <div className="text-xs text-slate-500 mt-1">Vc = cutting speed (m/min), D = tool Ø (mm)</div>
              </div>
              <div className="bg-slate-50 rounded-2xl border border-slate-100 p-4">
                <div className="text-xs font-black text-slate-400 uppercase mb-2">Feed Rate (mm/min)</div>
                <div className="font-mono text-sm text-slate-800 font-bold">F = RPM × fz × Z</div>
                <div className="text-xs text-slate-500 mt-1">fz = chip load/tooth, Z = number of flutes</div>
              </div>
              <div className="bg-slate-50 rounded-2xl border border-slate-100 p-4">
                <div className="text-xs font-black text-slate-400 uppercase mb-2">MRR (cm³/min)</div>
                <div className="font-mono text-sm text-slate-800 font-bold">MRR = (WOC × DOC × F) / 1000</div>
                <div className="text-xs text-slate-500 mt-1">WOC = width of cut, DOC = depth of cut</div>
              </div>
            </div>
          </div>
        </section>
