...
const SHORTCUTS = [
  ...
  { keys: 'Ctrl + 1', command: 'PROPERTIES', category: 'system', description: 'Toggle the Properties panel.' },
  { keys: 'Ctrl + 9', command: 'COMMAND LINE', category: 'system', description: 'Toggle the command line.' },
...
const TIPS = [
  {
    title: 'Accelerators tab for custom hotkeys',
    content: 'nanoCAD allows custom keyboard shortcuts (accelerators) via Tools > Customize > Interface > Accelerators tab. You can assign Ctrl+Shift combinations to frequently used commands for faster access.'
  },
  {
    title: 'Command aliases work like AutoCAD',
    content: 'nanoCAD supports AutoCAD-style command aliases. Type C for CIRCLE, L for LINE, etc. The alias file can be customized through Tools > Customize > Interface > Aliases tab.'
  },
];

export default function NanoCADShortcutsClient() {
  return (
    <ShortcutCheatsheetClient
      title="nanoCAD"
      subtitle="Searchable reference for nanoCAD keyboard shortcuts, hot keys, and command aliases — compatible with AutoCAD workflows."
      categories={CATEGORIES}
      shortcuts={SHORTCUTS}
      tips={TIPS}
      downloadAliasText={`; nanoCAD Command Aliases
L,*LINE
C,*CIRCLE
A,*ARC
REC,*RECTANGLE
...
B,*BLOCK
I,*INSERT`}
      downloadAliasFileName="nanocad_aliases.pgp"
    />
  );
}
