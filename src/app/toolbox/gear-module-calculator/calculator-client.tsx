    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          {solveFor !== 'module' && (
            <div>
              <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Module m (mm)</label>
              <input
                type="number"
                step="0.1"
                value={moduleVal}
          ...
          {solveFor !== 'teeth' && (
            <div>
              <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Teeth z₁</label>
              <input
                type="number"
                step="1"
                value={teeth}
          ...
          {solveFor !== 'pitchDiameter' && (
            <div>
              <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Pitch Diameter d (mm)</label>
              <input
                type="number"
                step="0.1"
                value={pitchDiameter}
          ...
          <div className="pt-2 border-t border-slate-50">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-xs font-bold text-slate-600">Calculate center distance (mating gear)</span>
            </label>
          </div>

          {showCenterDistance && (
            <div>
              <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Mating Gear Teeth z₂</label>
              <input
                type="number"
                step="1"
                value={teeth2}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
              ...
              {[
                ...
                >
                  ...
                  <p className={`text-xl font-black ${item.highlight ? 'text-blue-600' : 'text-slate-900'}`}>
                    {item.value}
                    {item.unit && <span className="text-xs text-slate-400 font-bold ml-1">{item.unit}</span>}
                  </p>
                </div>
              ))}
            </div>
            ...
            {showCenterDistance && (
              <div className="mt-6 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
                <p className="text-xs font-black uppercase tracking-wider opacity-80 mb-1">Center Distance (a = m × (z₁ + z₂) / 2)</p>
                <p className="text-3xl font-black">
                  {result.centerDistance.toFixed(4)}
                  <span className="text-sm font-bold ml-2 opacity-80">mm</span>
                </p>
              </div>
            )}
          </div>
          ...
          {/* SVG Visualization */}
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            <h3 className="text-sm font-black text-slate-900 mb-4">Gear Geometry Visualization</h3>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-black text-slate-900">Formula Reference</h3>
            </div>
