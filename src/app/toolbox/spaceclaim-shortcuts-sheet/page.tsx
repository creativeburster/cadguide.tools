      ...
      <main className="min-h-screen bg-slate-50 print:bg-white print:min-h-0">
        ...
        <section className="bg-white border-b border-slate-100 py-8 print:hidden">
          <div className="max-w-[1200px] mx-auto px-6 md:px-12">
            <h2 className="text-lg font-black text-slate-400 uppercase tracking-wider mb-4">Core Direct Modeling Tools</h2>
              ...
              {[
                ...
                <div key={tool.key} className="bg-amber-50 rounded-2xl border border-amber-100 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <kbd className="font-mono bg-slate-900 text-amber-400 px-3 py-1.5 rounded-lg font-black text-lg select-all">{tool.key}</kbd>
                    <span className="font-black text-slate-800">{tool.name}</span>
                  </div>
                  <p className="text-base text-slate-500 font-medium">{tool.desc}</p>
                </div>
              ))}
            </div>
          </div>
