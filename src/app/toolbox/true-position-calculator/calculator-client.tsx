    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Position Tolerance Ø (mm)</label>
          ...
          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">X Deviation (mm)</label>
            <input type="number" step="0.01" value={xOffset} onChange={e => setXOffset(parseFloat(e.target.value) || 0)}
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold" />
          </div>

          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Y Deviation (mm)</label>
          ...
          <div>
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-xs font-black text-slate-400 uppercase tracking-wider">Apply Bonus Tolerance</span>
            </label>
          </div>
          ...
          {useBonus && (
            <>
              <div>
                <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Material Condition</label>
                <div className="grid grid-cols-2 gap-2">
                  {(['MMC', 'LMC'] as const).map(m => (
                    <button key={m} onClick={() => setModifier(m)}
                      className={`px-3 py-2.5 rounded-xl text-xs font-black border transition-all ${modifier === m ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-100 text-slate-600'}`}>
                      {m}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">{modifier} Size (mm)</label>
                <input type="number" step="0.01" value={mmc} onChange={e => setMmc(parseFloat(e.target.value) || 0)}
                  className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-sm font-bold" />
              </div>
              <div>
                <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Actual Feature Size (mm)</label>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            ...
            <div className={`rounded-2xl p-6 border mb-4 ${result.isPass ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
              <div className={`text-xs font-black uppercase tracking-wider mb-2 ${result.isPass ? 'text-green-400' : 'text-red-400'}`}>
              ...
              <div className={`text-xs font-medium mt-1 ${result.isPass ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                <div className="text-xs font-black text-slate-400 uppercase mb-1">Specified Tol.</div>
                <div className="text-lg font-black text-slate-700">Ø{toleranceDia.toFixed(3)} mm</div>
              </div>
              {useBonus && (
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                  <div className="text-xs font-black text-slate-400 uppercase mb-1">Bonus Tol.</div>
                  <div className="text-lg font-black text-slate-700">+{result.bonus.toFixed(3)} mm</div>
                </div>
              )}
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                <div className="text-xs font-black text-slate-400 uppercase mb-1">Total Allowance</div>
                <div className="text-lg font-black text-slate-700">Ø{result.totalTolerance.toFixed(3)} mm</div>
              </div>
            </div>

            <div className="mt-4 bg-slate-50 rounded-xl p-4 border border-slate-100">
              <div className="text-xs font-black text-slate-400 uppercase mb-1">Tolerance Utilization</div>
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-black text-slate-700">{result.utilization.toFixed(1)}%</span>
              </div>
            </div>
