...
export default function OrificeFlowCalculatorClient() {
  const [inputs, setInputs] = useState(["100","50","10","1000","0.62"]);
  ...
  const result = useMemo(() => {
    const D=parseFloat(inputs[0])/1000,d=parseFloat(inputs[1])/1000,dP=parseFloat(inputs[2])*1000,rho=parseFloat(inputs[3]),Cd=parseFloat(inputs[4]);const A=Math.PI*d*d/4;const Q=Cd*A*Math.sqrt(2*dP/rho);const v=Q/(Math.PI*D*D/4);return{Q:Q*1000,v};
  }, [inputs]);
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          <div key="1">
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Orifice Dia (mm)</label>
            <input type="text" value={inputs[1]} onChange={e => updateInput(1, e.target.value)} placeholder="50"
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-lg font-bold focus:outline-none focus:ring-4 focus:ring-teal-600/5 focus:bg-white transition-all" />
          </div>
          <div key="2">
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Pressure Drop (kPa)</label>
          ...
          <div key="4">
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Discharge Cd</label>
            <input type="text" value={inputs[4]} onChange={e => updateInput(4, e.target.value)} placeholder="0.62"
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-lg font-bold focus:outline-none focus:ring-4 focus:ring-teal-600/5 focus:bg-white transition-all" />
          </div>

          <button onClick={() => setInputs(["100","50","10","1000","0.62"])}
