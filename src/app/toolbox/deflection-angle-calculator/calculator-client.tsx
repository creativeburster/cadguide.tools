...
export default function DeflectionAngleCalculatorClient() {
  const [inputs, setInputs] = useState(["10","5","2.5","200","5000"]);
  ...
  const result = useMemo(() => {
    const w=parseFloat(inputs[0])*1000,L=parseFloat(inputs[1]),x=parseFloat(inputs[2]),E=parseFloat(inputs[3])*1e9,I=parseFloat(inputs[4])*1e-8;const theta=w*x*(L*L-3*x*x)/(6*E*I);const delta=w*x*(L*L*L-2*L*x*x+x*x*x)/(24*E*I)*1000;return{theta,delta};
  }, [inputs]);
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          <div key="0">
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Load w (kN/m)</label>
          ...
          <div key="2">
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Position x (m)</label>
            <input type="text" value={inputs[2]} onChange={e => updateInput(2, e.target.value)} placeholder="2.5"
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-lg font-bold focus:outline-none focus:ring-4 focus:ring-orange-600/5 focus:bg-white transition-all" />
          </div>
          <div key="3">
          ...
          <button onClick={() => setInputs(["10","5","2.5","200","5000"])}
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-blue-50 rounded-2xl p-6 border border-blue-100">
                <div className="text-base font-black text-blue-400 uppercase tracking-wider mb-2">Slope</div>
                <div className="text-3xl font-black text-blue-700">{formatNum(result.theta)}<span className="text-lg text-blue-400"> rad</span></div>
              </div>
              <div className="bg-green-50 rounded-2xl p-6 border border-green-100">
                <div className="text-base font-black text-green-400 uppercase tracking-wider mb-2">Deflection</div>
                <div className="text-3xl font-black text-green-700">{formatNum(result.delta)}<span className="text-lg text-green-400"> mm</span></div>
              </div>
            </div>
            ...
            <div className="mt-6 bg-slate-50 rounded-2xl p-4 border border-slate-100">
              <div className="flex items-center gap-2 text-base text-slate-500 font-medium">
                ...
                For simply supported UDL: theta = w*x*(L^2-3x^2)/(6EI)
              </div>
            </div>
          </div>
