...
export default function ShaftDiameterCalculatorClient() {
  const [inputs, setInputs] = useState(["500","40","0.75"]);
  ...
  const result = useMemo(() => {
    const T=parseFloat(inputs[0]),tau=parseFloat(inputs[1])*1e6,kf=parseFloat(inputs[2]);const dSolid=Math.pow(16*T/(Math.PI*tau),1/3)*1000;const d=dSolid/kf;return{d:Math.ceil(d),dSolid:Math.ceil(dSolid)};
  }, [inputs]);
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          <div key="1">
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Shear Stress (MPa)</label>
            <input type="text" value={inputs[1]} onChange={e => updateInput(1, e.target.value)} placeholder="40"
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-lg font-bold focus:outline-none focus:ring-4 focus:ring-blue-600/5 focus:bg-white transition-all" />
          </div>
          <div key="2">
            <label className="text-base font-black text-slate-400 uppercase tracking-wider block mb-2">Keyway Factor</label>
            <input type="text" value={inputs[2]} onChange={e => updateInput(2, e.target.value)} placeholder="0.75"
              className="w-full h-12 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-lg font-bold focus:outline-none focus:ring-4 focus:ring-blue-600/5 focus:bg-white transition-all" />
          </div>

          <button onClick={() => setInputs(["500","40","0.75"])}
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-green-50 rounded-2xl p-6 border border-green-100">
                <div className="text-base font-black text-green-400 uppercase tracking-wider mb-2">Solid Shaft</div>
                <div className="text-3xl font-black text-green-700">{formatNum(result.dSolid)}<span className="text-lg text-green-400"> mm</span></div>
              </div>
            </div>
