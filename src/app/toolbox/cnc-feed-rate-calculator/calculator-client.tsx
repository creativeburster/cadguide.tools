    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 p-8 shadow-sm space-y-6">
          ...
          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Workpiece Material</label>
            <select
              value={materialIdx}
          ...
          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Tool Diameter D (mm)</label>
            <input
              type="number"
              step="0.5"
              value={toolDiameter}
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {[3, 4, 6, 8, 10, 12, 16, 20].map(d => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-slate-50 text-slate-500 hover:bg-blue-50 hover:text-blue-600 border border-slate-100 transition-all"
                >
                  Ø{d}
                </button>
              ))}
          ...
          <div>
            <label className="text-xs font-black text-slate-400 uppercase tracking-wider block mb-2">Number of Flutes z</label>
            <div className="grid grid-cols-4 gap-2">
              {[1, 2, 3, 4].map(f => (
                <button
                  key={f}
                  onClick={() => setFlutes(f)}
                  className={`h-12 rounded-xl text-sm font-black border transition-all ${
                    flutes === f
          ...
          <div className="flex items-center gap-2 text-xs font-bold text-slate-400 bg-slate-50 rounded-xl px-4 py-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 p-6 text-white">
                <p className="text-[10px] font-black uppercase tracking-wider opacity-80 mb-1">Spindle Speed (n)</p>
                <p className="text-3xl font-black">{results.rpm.toLocaleString()}<span className="text-sm font-bold ml-2 opacity-80">RPM</span></p>
                <p className="text-[10px] mt-2 opacity-70">n = Vc × 1000 / (π × D)</p>
              </div>
              <div className="rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 p-6 text-white">
                <p className="text-[10px] font-black uppercase tracking-wider opacity-80 mb-1">Feed Rate (Vf)</p>
                <p className="text-3xl font-black">{results.feedRate.toLocaleString()}<span className="text-sm font-bold ml-2 opacity-80">mm/min</span></p>
                <p className="text-[10px] mt-2 opacity-70">Vf = fz × z × n</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            <h3 className="text-sm font-black text-slate-900 mb-4">Recommended Cutting Parameters</h3>
            ...
            <p className="text-[10px] text-slate-400 mt-3 font-medium">Values are starting recommendations for HSS/Carbide end mills. Always consult tool manufacturer data and adjust based on rigidity, coolant, and depth of cut.</p>
          </div>
        </div>
      </div>
