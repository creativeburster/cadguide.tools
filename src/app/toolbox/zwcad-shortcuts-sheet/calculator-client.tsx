...
const TIPS = [
  {
    title: 'ZWCAD aliases are PGP-compatible with AutoCAD',
    content: 'ZWCAD uses the same PGP file format as AutoCAD. If you are migrating, you can import your existing acad.pgp file directly via the Customize User Interface dialog. Most one-key and two-key aliases are identical.'
  },
  {
    title: 'SmartSelect and SmartMouse features',
    content: 'ZWCAD includes SmartSelect (right-click context menus based on object type) and SmartMouse gestures. Enable them in Tools > Options > User Preferences to get AutoCAD-like behavior plus gesture shortcuts.'
  },
];

export default function ZWCADShortcutsClient() {
  return (
    <ShortcutCheatsheetClient
      title="ZWCAD"
      subtitle="Searchable reference for ZWCAD keyboard shortcuts, function keys, and PGP-compatible command aliases for efficient 2D/3D drafting."
      categories={CATEGORIES}
      shortcuts={SHORTCUTS}
      tips={TIPS}
      downloadAliasText={`; ZWCAD Command Aliases
L,*LINE
C,*CIRCLE
A,*ARC
REC,*RECTANGLE
...
B,*BLOCK
I,*INSERT`}
      downloadAliasFileName="zwcad_aliases.pgp"
    />
  );
}
