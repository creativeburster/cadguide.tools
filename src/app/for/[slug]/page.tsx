      ...
      <main className="min-h-screen bg-slate-50">
        <article className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
          ...
          <header className="mb-10">
            ...
            {fullList.length > PERSONA_LIMIT && (
              <p className="mt-3 text-sm text-slate-500">
                Showing the top {PERSONA_LIMIT} of {fullList.length}. See the full list in the{' '}
          ...
          <section className="mt-12 rounded-2xl bg-blue-600 text-white p-8 text-center">
            <h2 className="text-2xl font-bold">Get a personalised shortlist</h2>
            <p className="mt-3 text-blue-100 max-w-xl mx-auto">Our Matchmaker filters all {fullList.length} {p.displayName.toLowerCase()}-friendly tools against your real budget and feature needs in 60 seconds.</p>
          ...
          <section className="mt-10">
            ...
            <ul className="flex flex-wrap gap-2 text-sm">
              {Object.values(PERSONA_PAGES)
                ...
                .map((x) => (
                  <li key={x.slug}>
                    <Link href={`/for/${x.slug}`} className="px-3 py-1.5 rounded-full bg-white border border-slate-200 text-slate-700 hover:border-blue-300 hover:text-blue-700">
