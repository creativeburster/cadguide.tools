...
export function ToolDetailClient({ tool, category, alternativeTools }: Props) {
  ...
  const menuItems = [
    ...
    { id: 'pricing', label: 'Pricing Plans', icon: <CreditCard className="w-4 h-4" /> },
    { id: 'features', label: 'Key Features', icon: <Zap className="w-4 h-4" /> },
    { id: 'specs', label: 'Tech Specs', icon: <BarChart3 className="w-4 h-4" /> },
    { id: 'reviews', label: 'Pros & Cons', icon: <MessageSquare className="w-4 h-4" /> },
    { id: 'faq', label: 'FAQ', icon: <HelpCircle className="w-4 h-4" /> },
    { id: 'alternatives', label: 'Alternatives', icon: <Search className="w-4 h-4" /> }
  ];
  ...
  const allRelatedIds = [tool.id, ...alternativeTools.filter(Boolean).map(a => a!.id)].join(',');
      ...
      <div className="max-w-[1360px] mx-auto px-4 py-12">
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          ...
          <main className="flex-1 space-y-20 min-w-0">
            ...
            {/* Hero Section */}
            <section id="overview" className="bg-white rounded-[48px] p-8 md:p-14 border border-slate-100 shadow-sm relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex flex-col md:flex-row items-start gap-10 mb-12">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-3 mb-6">
                      ...
                      <Badge variant="outline" className="bg-slate-50 text-slate-500 border-slate-100 font-bold px-4">
                        {tool.pricing_type}
                      </Badge>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-8">
                      <div className="flex items-center gap-2 bg-yellow-50 px-5 py-2 rounded-2xl border border-yellow-100">
                        <span className="text-yellow-600 font-black text-2xl">★ {tool.score}</span>
                        <span className="text-yellow-700/50 text-[10px] font-black tracking-widest uppercase">Expert Score</span>
                      </div>
                      <div className="flex items-center gap-3 text-slate-500 font-bold">
                        <Globe className="w-5 h-5 text-blue-500" /> 
                        <span className="text-sm">Origin: <span className="text-slate-900">{tool.country}</span></span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="prose prose-slate max-w-none">
                  ...
                  <div className="flex flex-wrap items-center gap-4 mb-10">
                    <Button asChild className="rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-black h-14 px-10 shadow-xl shadow-blue-200">
                       <a href={tool.affiliate_url || tool.official_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-lg">
                        Go to Website <ExternalLink className="w-5 h-5" />
                      </a>
                    </Button>
                    <Button asChild variant="outline" className="rounded-2xl border-blue-100 text-blue-600 hover:bg-blue-50 font-black h-14 px-8 text-lg">
                      <Link href={`/compare?ids=${tool.id}`} className="flex items-center gap-3">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {tool.core_features?.map((feat, i) => (
                      <span key={i} className="bg-slate-50 text-slate-600 px-4 py-2 rounded-xl text-sm font-black border border-slate-100">#{feat}</span>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                { icon: <Cpu className="w-6 h-6" />, label: 'Geometry Kernel', value: tool.tech_specs?.engine },
                { icon: <Layers className="w-6 h-6" />, label: 'Industry Standards', value: tool.tech_specs?.standards?.slice(0,2).join(', ') },
                { icon: <ShieldCheck className="w-6 h-6" />, label: 'Market Seg.', value: tool.user_scales?.[0] },
                { icon: <CreditCard className="w-6 h-6" />, label: 'Starting At', value: `$${tool.starting_price}` },
              ].map((stat, i) => (
                <div key={i} className="bg-white p-8 rounded-[36px] border border-slate-100 shadow-sm flex flex-col items-center text-center transition-all hover:border-blue-200 hover:-translate-y-1">
                  ...
                  <div className="font-black text-slate-900 text-base">{stat.value || 'N/A'}</div>
                </div>
              ))}
            </div>
            ...
            <section id="pricing" className="scroll-mt-36 space-y-10">
              <div className="flex items-center gap-5">
                ...
                <h2 className="text-4xl font-black text-slate-900 tracking-tight">Full Pricing Breakdown</h2>
              </div>
              
              <div className="grid md:grid-cols-3 gap-8">
                {tool.pricing_tiers?.map((tier, i) => (
                  <Card key={i} className={`rounded-[44px] overflow-hidden border-2 transition-all hover:shadow-2xl ${tier.is_popular ? 'border-blue-600 shadow-xl scale-[1.03]' : 'border-slate-100'}`}>
                    {tier.is_popular && (
                    ...
                    <CardHeader className="text-center p-10 pb-6">
                      <div className="text-slate-400 font-black uppercase text-[10px] tracking-widest mb-4">{tier.name}</div>
                      <div className="flex items-baseline justify-center gap-1">
                        <span className="text-5xl font-black text-slate-900">${tier.price}</span>
                        <span className="text-slate-400 font-bold text-sm tracking-tight">{tier.period}</span>
                      </div>
                    </CardHeader>
                    <CardContent className="p-10 pt-4">
                      ...
                      <ul className="space-y-5 mb-10">
                        {tier.features?.map((f, j) => (
                          <li key={j} className="flex items-start gap-4 text-sm font-bold text-slate-600 leading-tight">
            ...
            <section id="features" className="scroll-mt-36 space-y-10">
              <div className="flex items-center gap-5">
                ...
                <h2 className="text-4xl font-black text-slate-900 tracking-tight">Key Capabilities</h2>
              </div>
                ...
                {tool.detailed_features?.map((cat, i) => (
                  <div key={i} className="p-6 md:p-8">
                    <h3 className="font-black text-slate-900 mb-4 uppercase text-[10px] tracking-[0.2em] text-slate-400 flex items-center gap-4">
                      {cat.category} <div className="h-px bg-slate-100 flex-1"></div>
                    </h3>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-8 gap-y-3">
                      {cat.items?.map((item, j) => (
                        <div key={j} className="flex items-center gap-2 group">
                          ...
                          <span className="text-sm font-bold text-slate-600 group-hover:text-blue-600 transition-colors leading-tight">{item.name}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <section id="specs" className="scroll-mt-36 space-y-10">
              <div className="flex items-center gap-5">
                ...
                <h2 className="text-4xl font-black text-slate-900 tracking-tight">Technical Audit</h2>
              </div>
              <div className="grid md:grid-cols-2 gap-8">
                <Card className="rounded-[40px] p-10 border-slate-100 shadow-sm">
                  <h4 className="font-black text-slate-900 mb-8 uppercase text-[10px] tracking-widest text-slate-400">Environment Support</h4>
                  <div className="space-y-6">
                    <div className="flex justify-between items-center text-base">
                      <span className="font-bold text-slate-500">Certified OS</span>
                      <span className="font-black text-slate-900">{tool.platforms.join(', ')}</span>
                    </div>
                    <div className="flex justify-between items-center text-base">
                      <span className="font-bold text-slate-500">Multi-threading</span>
                      <span className="font-black text-slate-900">{tool.tech_specs?.multicore}</span>
                    </div>
                    <div className="flex justify-between items-center text-base">
                      <span className="font-bold text-slate-500">GPU Optimization</span>
                      <span className="font-black text-slate-900">{tool.tech_specs?.gpu_optimization}</span>
                    </div>
                  </div>
                </Card>
                <Card className="rounded-[40px] p-10 border-slate-100 shadow-sm">
                  <h4 className="font-black text-slate-900 mb-8 uppercase text-[10px] tracking-widest text-slate-400">Industry Standards</h4>
                  <div className="flex flex-wrap gap-3">
                    {tool.tech_specs?.standards.map(s => (
                      <Badge key={s} className="bg-slate-50 text-slate-600 border-slate-100 font-black px-4 py-2 rounded-xl">{s}</Badge>
                    ))}
                  </div>
                </Card>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-12 rounded-[48px] border-4 border-green-50 shadow-sm">
                  ...
                  <ul className="space-y-5">
                    {tool.pros.map((pro, i) => (
                      <li key={i} className="flex items-start gap-5 p-5 bg-green-50/40 rounded-[24px] text-base font-black text-slate-700 leading-snug">
                ...
                <div className="bg-white p-12 rounded-[48px] border-4 border-red-50 shadow-sm">
                  ...
                  <ul className="space-y-5">
                    {tool.cons.map((con, i) => (
                      <li key={i} className="flex items-start gap-5 p-5 bg-red-50/40 rounded-[24px] text-base font-black text-slate-700 leading-snug">
              ...
              <div className="bg-slate-900 text-white rounded-[56px] p-16 relative overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-5 mb-12">
                    ...
                    <div>
                      <h3 className="text-3xl font-black tracking-tight">CADTools Verdict</h3>
                      <p className="text-blue-400 font-black uppercase text-[10px] tracking-[0.4em] mt-1">Final Professional Conclusion</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between border-t border-white/5 pt-12 gap-8">
                    ...
                    <Badge className="bg-blue-600 text-white font-black px-6 py-2 rounded-xl text-sm">Highly Recommended</Badge>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section id="faq" className="scroll-mt-36 space-y-10">
              <div className="flex items-center gap-5">
                ...
                <h2 className="text-4xl font-black text-slate-900 tracking-tight">Expert Q&A</h2>
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                {tool.faqs.map((faq, i) => (
                  <div key={i} className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm group hover:border-blue-200 transition-all">
                    ...
                    <p className="text-slate-600 text-base leading-relaxed font-medium pl-8">{faq.a}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section id="alternatives" className="scroll-mt-36 space-y-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-5">
                  ...
                  <h2 className="text-4xl font-black text-slate-900 tracking-tight">Compare Alternatives</h2>
                </div>
                <Button asChild variant="outline" className="rounded-2xl border-blue-100 text-blue-600 font-black h-12 px-8 hover:bg-blue-50">
                  <Link href={`/compare?ids=${allRelatedIds}`} className="flex items-center gap-2">
                    <Scale className="w-4 h-4" /> Compare All {alternativeTools.length + 1} Tools
                  </Link>
                </Button>
              </div>
              <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="sticky top-28 space-y-10">
              ...
              <Card className="rounded-[40px] border-2 border-blue-600 shadow-2xl shadow-blue-900/10 overflow-hidden flex flex-col group bg-[#0f172a]">
                <div className="bg-[#0f172a] text-white p-10 pb-8 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="text-blue-400 font-black uppercase text-[10px] tracking-[0.3em] mb-4">Official Release</div>
                    <div className="text-3xl md:text-4xl font-black leading-tight">
                      Get Licensed <br/>{tool.name}
                    </div>
                  </div>
                </div>
                <CardContent className="p-10 flex-1 bg-white relative z-10">
                  <div className="flex items-baseline gap-2 mb-10">
                    <span className="text-slate-400 text-lg font-bold">From</span>
                    <span className="text-5xl font-black text-slate-900">${tool.starting_price}</span>
                  ...
                  <Button asChild className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black h-20 rounded-[24px] shadow-2xl shadow-blue-200 text-xl transition-all active:scale-95">
                    <a href={tool.affiliate_url || tool.official_url} target="_blank" rel="noopener noreferrer">
                      Buy Now
                    </a>
                  </Button>
                  <p className="text-center text-[10px] text-slate-300 mt-8 font-black uppercase tracking-[0.2em]">Verified Vendor Link</p>
                </CardContent>
              </Card>
              ...
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-10 rounded-[48px] text-white relative overflow-hidden shadow-xl shadow-blue-200 group">
                ...
                <div className="relative z-10">
                  ...
                  <h4 className="text-2xl font-black mb-4 leading-tight">Need a better match?</h4>
                  <p className="text-blue-100 text-sm font-medium mb-8 leading-relaxed">
                    Our AI-powered Matchmaker can find you a more efficient tool in under 60 seconds.
                  </p>
                  <Button asChild className="w-full bg-white text-blue-600 hover:bg-blue-50 font-black rounded-2xl h-14 shadow-lg shadow-blue-900/20">
              ...
              <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-sm">
                <div className="flex items-center gap-3 mb-8">
                   ...
                   <h4 className="text-xl font-black text-slate-900 tracking-tight">Trending AEC Tools</h4>
                </div>
                <div className="space-y-6">
                  {trendingTools.map(t => (
                ...
                <Button asChild variant="link" className="w-full text-blue-600 font-black text-xs uppercase tracking-widest p-0 h-auto">
