...
excerpt: "Switching from V-Ray CPU to GPU rendering often introduces noise and fireflies that won't resolve with more samples. I cover the exact Noise Threshold, Light Cache subdivs, and Fireflies Filter settings that eliminate persistent noise on GPU renders."
...
I switched our studio from V-Ray CPU to V-Ray GPU 6 last year, and the first thing I noticed was that my renders had more noise than before — not less, which is what I expected from a modern GPU renderer. No matter how much I lowered the Noise Threshold or increased the sample count, certain scenes still came out with splotchy noise and bright fireflies. After weeks of testing and reading through Chaos forum threads, I've developed a settings approach that consistently produces clean GPU renders.
...
**My standard settings for GPU rendering**:
...
A Chaos forum user reported that even with Noise Threshold at 0.005 and Subdivs at 3000, they still couldn't eliminate noise and fireflies. The issue wasn't the sampling settings — it was the Light Cache and the Fireflies Filter. This is why I never rely on sampling alone.
...
Increasing Light Cache subdivs has a relatively small impact on render time compared to increasing the main sampling settings, but it dramatically reduces splotchy noise. I always increase Light Cache before touching the Noise Threshold.
...
I enable the Fireflies Filter on every GPU render. It has minimal render time cost and eliminates 90% of firefly issues. For the remaining 10%, I use the V-Ray Denoiser.

## Setting 4: V-Ray Denoiser

The Denoiser is the most effective tool for clean GPU renders, and it's been significantly improved in V-Ray 6. Instead of spending hours increasing sample counts, I render at a higher Noise Threshold (faster) and let the Denoiser clean up the remaining noise.
...
**Workflow**: I render with Noise Threshold at 0.01 (default) and let the Denoiser clean it up. This gives me clean results in 1/3 the time compared to rendering with Noise Threshold at 0.003 without denoising.
...
- **Type**: I use **Catmull-Rom** for sharp results (good for architectural visualization)
...
Cosmos HDRI maps are popular for lighting, but they can introduce noise if the HDRI has extremely bright spots. I've seen fireflies specifically from HDRI maps with visible sun disks.
...
Sometimes noise is concentrated in one material rather than the entire scene. I had a wood-lined ceiling that was always noisy while the rest of the scene was clean. The issue was the material's reflection glossiness.
...
I've noticed that V-Ray GPU produces slightly different noise patterns than CPU. GPU noise tends to be more splotchy (larger clumps) while CPU noise is more uniform (fine grain). This is because GPU uses a different sampling algorithm optimized for parallel processing.
