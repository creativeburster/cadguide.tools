...
excerpt: "ZBrush files exceeding 4GB cause crashes, slow saves, and export failures. We cover the decimation workflow, baking detail into displacement maps, and the SubTool splitting strategy that keeps projects manageable without sacrificing visual quality."
...
The decimated model retains the visual detail of the original through normal and displacement maps (which we bake before decimating). The exported file is 1/100th the size of the original.
...
Instead of exporting a 50-million-poly model, we bake the detail into maps and export a 5,000-poly model with displacement and normal maps. The final render looks identical, but the file is 10,000x smaller.
...
**Practical example**: A character head sculpt at 80 million polygons is one SubTool. We split it into:
- Head base: 30M polys
...
A `.ztl` (ZTool) file saves only the model data for a single tool. We save each SubTool as a separate `.ztl` file:
...
When we need to resume work, we create a new project and load each `.ztl` file with **Tool → Load Tool**. This approach has three advantages:
- Individual `.ztl` files are smaller and save faster
- If one file gets corrupted, we only lose one SubTool, not the entire project
- we can share individual SubTools with team members without sending a 6GB project file
...
**Caution**: Del Lower is irreversible. Only do this when you're certain you won't need to go back to a lower level. We typically keep levels 1, 2, and the current highest level, and delete the intermediate ones.

## Strategy 6: Clean Up Before Saving

Before saving any ZBrush file, we run through this cleanup checklist:
...
5. **Set the canvas to a small size**: Large canvas dimensions increase file size — We set it to 500x500 before saving
...
- **GoZ**: If you have GoZ configured, it exports directly to connected applications (Maya, 3ds Max, Blender) with one click. We use GoZ for quick iteration between ZBrush and 3ds Max

For extremely high-poly exports that crash on OBJ export, decimate first. We've never had a decimated model fail to export.
